...
export const RouteGuard: React.FC<{children: React.ReactNode}> = ({children})=>{
  ...
  useEffect(()=>{
    if (!settings.enabled) { setBlocked(null); return; }
    const now = new Date();

    // 一時解除中はスルー
    if (tempUnlockedUntil && Date.now() < tempUnlockedUntil) { setBlocked(null); return; }

    // Panic
    if (isPanic) { setBlocked(loc.pathname); return; }

    // スケジュール該当？
    const inAny = settings.schedules.some(s=>isNowInSchedule(now, s));
    if (!inAny) { setBlocked(null); return; }

    // ルート一致？
    const hit = settings.blockRoutes.some(p => loc.pathname.startsWith(p));
    if (hit) { setBlocked(loc.pathname); return; }
  ...
  const unlock = (minutes:number, reason:string) => {
    ...
    const arr = JSON.parse(localStorage.getItem('guard:unlockLogs')||'[]'); arr.push(rec);
