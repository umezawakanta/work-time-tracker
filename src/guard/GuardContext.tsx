...
const DEFAULT: GuardSettings = {
  ...
  schedules: [], blockRoutes: [], blockCategories: [],
  whitelistDuringFocus: [], panicUntil: null
};
...
export const GuardProvider: React.FC<{children: React.ReactNode}> = ({children}) => {
  ...
  useEffect(() => { localStorage.setItem('guard:settings', JSON.stringify(settings)); }, [settings]);
  useEffect(() => {
  ...
  const value = useMemo<Ctx>(()=>({settings,setSettings,isPanic,tempUnlockedUntil,setTempUnlockedUntil}),[settings,isPanic,tempUnlockedUntil]);
