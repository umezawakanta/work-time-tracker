    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl">
        ...
        <p className="text-sm text-gray-600 mb-4">
          パス: <span className="font-mono">{blockedPath}</span>
        </p>
        ...
        {settings?.panicUntil && (
          <p className="text-xs text-red-600 mt-3">※ Panicモード中は解除できない場合があります</p>
        )}
      </div>

      {/* 遅延カウントダウン */}
      {delay > 0 && <Countdown seconds={delay} onTick={(s) => setDelay(s)} />}
    </div>
  );
};

const Countdown: React.FC<{ seconds: number; onTick: (s: number) => void }> = ({
  seconds,
  onTick,
}) => {
