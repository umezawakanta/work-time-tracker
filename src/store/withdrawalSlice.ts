...
export const deleteWithdrawalEntry = createAsyncThunk<
  string,
  string,
  { rejectValue: string }
>("withdrawal/deleteEntry", async (id, { rejectWithValue }) => {
  try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchWithdrawalEntries.rejected, (state, action) => {
        state.status = "failed";
        state.error =
          action.payload || "口座引き落とし情報の取得に失敗しました";
      })
      .addCase(
        addWithdrawalEntry.fulfilled,
        (state, action: PayloadAction<WithdrawalEntry>) => {
      ...
      .addCase(addWithdrawalEntry.rejected, (state, action) => {
        state.error =
          action.payload || "口座引き落とし情報の追加に失敗しました";
      })
      .addCase(
        updateWithdrawalEntry.fulfilled,
        (state, action: PayloadAction<WithdrawalEntry>) => {
      ...
      .addCase(updateWithdrawalEntry.rejected, (state, action) => {
        state.error =
          action.payload || "口座引き落とし情報の更新に失敗しました";
      })
      .addCase(
        deleteWithdrawalEntry.fulfilled,
      ...
      .addCase(deleteWithdrawalEntry.rejected, (state, action) => {
        state.error =
          action.payload || "口座引き落とし情報の削除に失敗しました";
      });
  },
});
