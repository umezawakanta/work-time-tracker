...
export const fetchAssetEntries = createAsyncThunk<AssetEntry[], void, { rejectValue: string }>(
  'asset/fetchEntries',
...
>('asset/addEntry', async (entry, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(
      error instanceof Error ? error.message : '資産エントリーの追加に失敗しました'
    );
  }
});
...
>('asset/updateEntry', async ({ id, entry }, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(
      error instanceof Error ? error.message : '資産エントリーの更新に失敗しました'
    );
  }
});

export const deleteAssetEntry = createAsyncThunk<string, string, { rejectValue: string }>(
  'asset/deleteEntry',
  async (id, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error instanceof Error ? error.message : '資産エントリーの削除に失敗しました'
      );
    }
  }
);
...
const assetSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchAssetEntries.fulfilled, (state, action: PayloadAction<AssetEntry[]>) => {
      ...
      .addCase(addAssetEntry.fulfilled, (state, action: PayloadAction<AssetEntry>) => {
