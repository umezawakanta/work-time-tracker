...
export default function SiteMapPage() {
  ...
  const grouped: Grouped = useMemo(() => {
    ...
    const filtered = featuresRegistry
      .filter((f) => !f.disabled)
      .filter((f) => {
        const hit = (f.name + f.description + f.category + f.path)
          .toLowerCase()
          ...
          {categories.map((c) => (
            <li key={c}>
              <a
                ...
                className="inline-block whitespace-nowrap rounded-xl border px-3 py-1 text-sm hover:bg-slate-50"
              >
                {c}
              </a>
            </li>
                ...
                {items.map((f) => (
                  <li key={f.id} className="rounded-xl border p-3 hover:shadow-sm transition">
                    <div className="flex items-start justify-between gap-2">
                      <Link
                        to={f.path}
                        className="font-medium underline-offset-2 hover:underline break-words"
                      >
                        {f.name}
                      </Link>
                      <div className="flex gap-1 shrink-0">
                        ...
                        {f.priority && (
                          <Badge
                            tone={
                              f.priority === 'P0' ? 'bad' : f.priority === 'P1' ? 'warn' : 'default'
                            }
                          >
                            {f.priority}
                          </Badge>
