...
const GuardPage: React.FC = () => {
  ...
  const addNight = () => setLocal(s=>({
    ...s, schedules: [...s.schedules, {days:[0,1,2,3,4,5,6], start:'22:00', end:'07:00', strict:true}]
  }));
        ...
        <div className="rounded-2xl border p-4 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              onChange={e=>setLocal(s=>({...s, blockRoutes: e.target.value.split('\n').filter(Boolean)}))}
          ...
          <div>
            ...
            {local.schedules.map((sc, i)=>(
              <div key={i} className="flex items-center gap-2 text-sm mb-2">
                ...
                <input 
                  ...
                  value={sc.start} 
                  onChange={e=>{
                    const v=[...local.schedules]; v[i]={...v[i],start:e.target.value}; setLocal(s=>({...s,schedules:v}));
                  }}
                  placeholder="22:00"
                  aria-label="開始時間"
                />
                ...
                <input 
                  ...
                  value={sc.end} 
                  onChange={e=>{
                    const v=[...local.schedules]; v[i]={...v[i],end:e.target.value}; setLocal(s=>({...s,schedules:v}));
                  }}
                  placeholder="07:00"
                  aria-label="終了時間"
                />
            ...
            <button onClick={addNight} className="mt-1 h-10 px-3 rounded-lg border">+ 夜間22-7を追加</button>
          </div>

          <div className="flex gap-3">
            <button onClick={()=>setLocal({...settings})} className="h-11 px-4 rounded-xl border">戻す</button>
            <button onClick={save} className="h-11 px-4 rounded-xl bg-indigo-600 text-white">保存</button>
          </div>
        </div>
      </main>
    </>
