...
export function AssetLiabilityTrendChart({
  ...
  const processedData = useMemo(() => {
    ...
    // 口座ごとの日付→値のマッピングを作成
    const accountData: Record<string, Record<string, { value: number; type: 'asset' | 'debt' }>> = {};
  ...
  const aggregatedData = useMemo(() => {
    ...
    Object.keys(aggregated).forEach((dateStr) => {
      ...
      // 資産合計
      entry.totalAssets = Object.values(entry.assets).reduce(
        (sum, value) => sum + value, 0
      );
      
      // 負債合計
      entry.totalDebts = Object.values(entry.debts).reduce(
        (sum, value) => sum + value, 0
      );
      ...
      <div className="current-balances">
        <div className="balance-section assets">
          ...
          {balanceSummary.assets.accounts.map((account) => (
            <div key={account.account} className="balance-card asset">
              ...
              <p className="balance-amount">
                {account.value.toLocaleString()} 円
              </p>
              <p
                className={`difference ${
                  account.change >= 0 ? "positive" : "negative"
                }`}
              >
                {account.change >= 0 ? "+" : ""}
              ...
              <p className="last-update">
                最終更新: {format(balanceSummary.date, "yyyy/MM/dd")}
              </p>
            </div>
          ))}
          <div className="balance-card asset-total">
            ...
            <p className="balance-amount">
              {balanceSummary.assets.total.toLocaleString()} 円
            </p>
            <p
              className={`difference ${
        ...
        <div className="balance-section liabilities">
          ...
          <div className="balance-card debt-total">
            <h3 className="account-name">合計</h3>
            <p className="balance-amount">
              {balanceSummary.debts.total.toLocaleString()} 円
            </p>
            <p
              className={`difference ${
                balanceSummary.debts.change <= 0 ? "positive" : "negative"
              }`}
            >
        ...
        <div className="balance-section total">
          ...
          <div className="balance-card net-worth">
            ...
            <p className="balance-amount">
              {balanceSummary.netWorth.total.toLocaleString()} 円
            </p>
            <p
              className={`difference ${
