...
const groupByDate = (entries: DebtEntry[]): GroupedDataItem[] => {
...
const filterDataByTimeRange = (data: GroupedDataItem[], timeRange: string): GroupedDataItem[] => {
  ...
  const cutoffDate = timeRange === 'month' 
    ? subMonths(now, 1) 
    : timeRange === 'quarter'
      ? subMonths(now, 3)
      : subYears(now, 1);
...
export const DebtTrendChart: React.FC<DebtTrendChartProps> = ({ data, timeRange }) => {
    ...
    if (active && payload && payload.length) {
      return (
        <Card className="p-2 bg-background border shadow-md">
          ...
          <p className="text-sm">
            <span className="mr-1">総負債:</span>
            <span className="font-semibold text-destructive">{formatYen(payload[0].value)}</span>
          </p>
        </Card>
      );
    }
