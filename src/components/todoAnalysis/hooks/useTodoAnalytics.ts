    ...
    const calculateAnalytics = async (): Promise<void> => {
      try {
        ...
        const completionRate =
          totalTasks > 0 ? Math.round((completedTodos.length / totalTasks) * 100) : 0;
        ...
        const categoryStats = todos.reduce(
          (acc, todo) => {
            ...
            if (
              ...
              (category.toLowerCase().includes('input') || category.toLowerCase() === 'インプット')
            ) {
              acc.input = (acc.input || 0) + 1;
            } else if (
              typeof category === 'string' &&
              (category.toLowerCase().includes('output') ||
                category.toLowerCase() === 'アウトプット')
            ) {
        ...
        Object.keys(categoryStats).forEach((key) => {
          categoryDistribution[key] = totalTasks > 0 ? categoryStats[key] / totalTasks : 0;
        });
        ...
        const recentTasks = todos.filter((todo) => {
        ...
        setState({
          ...
          isLoading: false,
          error:
            error instanceof Error ? error.message : '分析データの取得中にエラーが発生しました。',
        });
      }
    };
