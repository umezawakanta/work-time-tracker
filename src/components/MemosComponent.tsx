...
const MemosComponent: React.FC<MemosComponentProps> = (props) => {
  ...
  const isReplyLiked = (replyId: string): boolean => {
    const currentUserId = 'current-user-id'; // 実際のユーザーIDに置き換え
    const reply = memos.flatMap(memo => memo.replies || []).find(r => r.id === replyId);
    if (!reply) {
    return false;
  }
    return replyManagement.isReplyLiked(reply, currentUserId);
  };
    ...
    <div className="memos-section">
      ...
      <GenreManagementModal
        ...
        setCustomCategories={(categories) => {
          // カスタムカテゴリを更新する処理を追加
          console.log('Custom categories updated:', categories);
        }}
        deletedDefaultCategories={genreManagement.deletedDefaultCategories}
        setDeletedDefaultCategories={(categories) => {
          // 削除されたデフォルトカテゴリを更新する処理を追加
          console.log('Deleted default categories updated:', categories);
        }}
