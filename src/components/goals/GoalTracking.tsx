...
const formatRemainingTime = (days: number): string => {
  ...
  return remainingMonths > 0 ? `残り約${years}年${remainingMonths}ヶ月` : `残り約${years}年`;
};
...
const predictCompletionDate = (goal: FinancialGoal): string => {
  ...
  if ((goal.type === 'asset' && dailyChange <= 0) || (goal.type === 'debt' && dailyChange >= 0)) {
      ...
      <CardContent>
        <div className="space-y-4">
          <div>
            ...
            <Progress value={progress} className={`h-2 ${cardStyle.progressColor}`} />
          </div>

          <div className="grid grid-cols-2 gap-2 text-sm">
            <div>
              <div className="text-muted-foreground">現在</div>
              <div className="text-lg font-bold">¥{goal.currentValue.toLocaleString()}</div>
            </div>
            <div>
              <div className="text-muted-foreground">目標</div>
              <div className="text-lg font-bold">¥{goal.targetValue.toLocaleString()}</div>
            </div>
          </div>
...
// 目標トラッキングコンポーネント
export const GoalTracking: React.FC<GoalTrackingProps> = ({ goals, onAddGoal, onEditGoal }) => {
  // 期間によるフィルタリング
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">あなたの財務目標の進捗を追跡します</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-emerald-50 border-emerald-200">
          ...
          <CardContent>
            <div className="flex items-end gap-2">
              <div className="text-3xl font-bold text-emerald-700">{goalStats.completed}</div>
        ...
        <Card className="bg-blue-50 border-blue-200">
          ...
          <CardContent>
            <div className="flex items-end gap-2">
              <div className="text-3xl font-bold text-blue-700">{goalStats.onTrack}</div>
              <div className="text-blue-600 text-sm mb-1">
                {goalStats.total > 0 ? Math.round((goalStats.onTrack / goalStats.total) * 100) : 0}%
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-amber-50 border-amber-200">
          ...
          <CardContent>
            <div className="flex items-end gap-2">
              <div className="text-3xl font-bold text-amber-700">{goalStats.atRisk}</div>
              <div className="text-amber-600 text-sm mb-1">
                {goalStats.total > 0 ? Math.round((goalStats.atRisk / goalStats.total) * 100) : 0}%
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredGoals.length > 0 ? (
            filteredGoals.map((goal) => <GoalCard key={goal.id} goal={goal} onEdit={onEditGoal} />)
          ) : (
            ...
            <CardContent>
              <div className="space-y-4">
                {TIME_PERIODS.map((period) => {
                  const periodGoals = goals.filter((g) => g.period === period.value);
                  if (periodGoals.length === 0) return null;

                  return (
                    <div key={period.value} className="border-l-2 border-blue-200 pl-4 py-2">
                          ...
                          <div
                            className={`text-sm font-medium ${
                              prediction.includes('予測達成日')
                                ? 'text-green-600'
                                : prediction.includes('目標期日までに達成できない')
                                  ? 'text-red-600'
                                  : 'text-amber-600'
                            }`}
                          >
