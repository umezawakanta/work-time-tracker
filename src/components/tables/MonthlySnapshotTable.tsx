...
export const MonthlySnapshotTable: React.FC<MonthlySnapshotTableProps> = () => {
  ...
  const generateMonthlySnapshots = (): DataPoint[] => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      // 前月比の成長率 (最初の月は値なし)
      const assetGrowth = i > 0 
        ? ((baseAssets / snapshots[i-1].assets) - 1) * 100 
        : undefined;
      
      const debtGrowth = i > 0 
        ? ((baseDebts / snapshots[i-1].debts) - 1) * 100 
        : undefined;
      
      const netWorthGrowth = i > 0 
        ? ((netWorth / snapshots[i-1].netWorth) - 1) * 100 
        : undefined;
            ...
            <TableRow key={snapshot.date}>
              ...
              <TableCell className="text-right">
                {index === 0 ? (
                  ...
                  <Badge variant="default" className="bg-green-600">好調</Badge>
                ) : snapshot.netWorthGrowth && snapshot.netWorthGrowth < -2 ? (
