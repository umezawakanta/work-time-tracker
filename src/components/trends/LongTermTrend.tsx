...
export const LongTermTrend: React.FC<LongTermTrendProps> = ({
  ...
  const aggregatedData = useMemo(() => {
    ...
    // 長期間のデータは集計する
    if (['3years', '5years', '10years'].includes(timeRange) && chartData.length > 60) {
  ...
  const granularity = useMemo(() => {
    ...
    if (['month', 'quarter', 'halfyear', 'year', '2years'].includes(timeRange)) return 'month';
    return 'year';
  }, [timeRange]);
  ...
  const growthRates = useMemo(() => {
    ...
    // 変化率を計算
    const assetGrowth = first.assets > 0 ? ((last.assets - first.assets) / first.assets) * 100 : 0;

    const debtGrowth = first.debts > 0 ? ((last.debts - first.debts) / first.debts) * 100 : 0;

    const netWorthGrowth =
      first.netWorth > 0 ? ((last.netWorth - first.netWorth) / first.netWorth) * 100 : 0;
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">資産・負債の長期的な推移を確認できます</p>
        </div>
      ...
      {/* チャートタブ */}
      <Tabs defaultValue="netWorth" value={chartType} onValueChange={setChartType}>
        <TabsList>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <Card className="bg-blue-50 border-blue-200">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-blue-800">資産成長率</CardTitle>
              </CardHeader>
              <CardContent>
                ...
                <div className="text-xs text-blue-600">
                  {timeRange === 'day'
                    ...
                    : timeRange === 'week'
                      ...
                      : timeRange === 'month'
                        ? '過去12ヶ月'
                        : `過去の${TIME_RANGES.find((r) => r.value === timeRange)?.label || ''}`}
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-amber-50 border-amber-200">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-amber-800">負債変化率</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="bg-emerald-50 border-emerald-200">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-emerald-800">純資産成長率</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <CardDescription>時間経過による純資産（資産 - 負債）の変化</CardDescription>
                  </div>
                  {showGoals && (
              ...
              <CardFooter className="bg-muted/20 py-3">
                <div className="flex items-center text-sm text-muted-foreground">
                  ...
                  <span>
                    {aggregatedData.length > 0
                      ? `${formatDate(aggregatedData[0].date, 'day')} 〜 ${formatDate(
          ...
          <TabsContent value="assetComposition">
            <Card>
              <CardHeader>
                ...
                <CardDescription>時間経過による資産カテゴリの変化</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-96">
                  {aggregatedData.length > 0 && aggregatedData[0].categories ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={aggregatedData}>
                        ...
                        {/* 動的にカテゴリ別のエリアを生成 */}
                        {Object.keys(aggregatedData[0].categories || {}).map((category, index) => {
                          // 各カテゴリに異なる色を割り当て
                    ...
                    <div className="flex items-center justify-center h-full">
                      <p className="text-muted-foreground">カテゴリ別データがありません</p>
                    </div>
                  )}
                </div>
              </CardContent>
          ...
          <TabsContent value="debtComposition">
            <Card>
              <CardHeader>
                ...
                <CardDescription>時間経過による負債カテゴリの変化</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-96">
                  {aggregatedData.length > 0 && aggregatedData[0].categories ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={aggregatedData}>
                        ...
                        {/* 動的に負債カテゴリ別のエリアを生成 */}
                        {Object.keys(aggregatedData[0].debtCategories || {}).map(
                          (category, index) => {
                            // 各カテゴリに異なる色を割り当て
                    ...
                    <div className="flex items-center justify-center h-full">
                      <p className="text-muted-foreground">カテゴリ別負債データがありません</p>
                    </div>
                  )}
                </div>
              </CardContent>
      ...
      {aggregatedData.length > 0 && (
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>現在のトレンドに基づく将来予測と主要指標</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-3">
                  {aggregatedData.length > 1 && (
                    <>
                      <div className="flex justify-between items-center">
                        <span className="text-muted-foreground">年平均成長率（資産）</span>
                        <span className="font-medium">
                          {calculateAnnualGrowthRate(aggregatedData, 'assets')}%
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-muted-foreground">年平均減少率（負債）</span>
                        <span className="font-medium">
                          {calculateAnnualGrowthRate(aggregatedData, 'debts', true)}%
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-muted-foreground">年平均成長率（純資産）</span>
                        <span className="font-medium">
                          {calculateAnnualGrowthRate(aggregatedData, 'netWorth')}%
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-muted-foreground">負債/資産比率</span>
                        <span className="font-medium">
                      ...
                      {aggregatedData[0].savingsRate !== undefined && (
                        <div className="flex justify-between items-center">
                          <span className="text-muted-foreground">平均貯蓄率</span>
                          <span className="font-medium">
              ...
              <div>
                <h3 className="text-lg font-medium mb-3">将来予測（現在のトレンドに基づく）</h3>
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground">1年後の予測純資産</span>
                    <span className="font-medium">
                      {formatCurrency(predictFutureValue(aggregatedData, 'netWorth', 1))}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground">3年後の予測純資産</span>
                    <span className="font-medium">
                      {formatCurrency(predictFutureValue(aggregatedData, 'netWorth', 3))}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground">5年後の予測純資産</span>
                    <span className="font-medium">
                      {formatCurrency(predictFutureValue(aggregatedData, 'netWorth', 5))}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground">10年後の予測純資産</span>
                    <span className="font-medium">
                      {formatCurrency(predictFutureValue(aggregatedData, 'netWorth', 10))}
                    </span>
                  </div>
                </div>
              </div>
...
function calculateAnnualGrowthRate(
  ...
  if (isDebt) {
    // 負債の場合は減少率を正の値で表現
    growthRate = first[key] > 0 ? (((first[key] - last[key]) / first[key]) * 100) / yearsDiff : 0;
  } else {
    // 資産や純資産の成長率
    growthRate = first[key] > 0 ? (((last[key] - first[key]) / first[key]) * 100) / yearsDiff : 0;
  }
...
function calculateAverageSavingsRate(data: FinancialData[]): string {
  ...
  const avgRate = savingsRates.reduce((sum, rate) => sum + rate, 0) / savingsRates.length;
