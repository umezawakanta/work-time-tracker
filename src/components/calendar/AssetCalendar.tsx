...
import { Trash2, PlusCircle, Calendar, TrendingUp, ArrowUpCircle, ArrowDownCircle, RefreshCw } from "lucide-react";
...
// サマリーカードコンポーネント
const SummaryCard = ({ title, value, isPositive, icon: Icon, previous, showTrend = true }) => {
  const percentChange = previous && previous !== 0 ? ((value - previous) / Math.abs(previous)) * 100 : 0;
          ...
          {showTrend && previous !== undefined && (
            <div className="flex items-center mt-2">
              ...
              <span className={`text-sm ${
                ...
                {percentChange !== 0 ? `${Math.abs(percentChange).toFixed(1)}%` : '0%'}
                {percentChange > 0 ? ' 増加' : percentChange < 0 ? ' 減少' : ' 変化なし'}
              </span>
            </div>
          )}
        </CardContent>
  ...
  const events = useMemo(() => {
    ...
    return sortedDates.map((date) => {
      ...
      const dateSubscriptions = subscriptions.filter((s) => {
        ...
        const subscriptionDateStr = s.billingDate.includes('/') 
          ? s.billingDate 
          : s.billingDate.replace(/-/g, '/');
        const subscriptionDate = parse(subscriptionDateStr, 'yyyy/MM/dd', new Date());
  ...
  const renderEventContent = (eventInfo: EventContentArg) => {
    ...
    return (
      <div className="event-content">
        {hasAggregatedData && (
          <>
            <div className="total-amount">残高: {currentTotal.toLocaleString()}円</div>
        ...
        {(withdrawals.length > 0 || subscriptions.length > 0) && (
          <div className="total-expenses-today">
            ...
            <span className="expenses-value">{totalExpensesToday.toLocaleString()}円</span>
          </div>
        )}
...
return (
  <div className="asset-calendar-container">
    <Tabs defaultValue="summary" value={activeTab} onValueChange={setActiveTab} className="w-full mb-6">
      ...
      <TabsContent value="summary" className="space-y-6">
        <div className="flex flex-col space-y-4">
          <h2 className="text-2xl font-bold">{format(currentMonth, "yyyy年M月", { locale: ja })}の財務概要</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>サブスクリプションと引き落とし</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="mb-4">
                  ...
                  {topExpensiveWithdrawals.length > 0 ? (
                    <div className="space-y-2">
                      {topExpensiveWithdrawals.map((withdrawal) => (
                        <div key={withdrawal._id} className="flex justify-between items-center text-sm p-2 bg-gray-50 rounded">
                          <span>{withdrawal.description}</span>
                          <span className="font-medium">{withdrawal.amount.toLocaleString()}円</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-sm text-muted-foreground">この月の引き落としはありません</p>
                  )}
                </div>
              </CardContent>
            </Card>
                ...
                {sortedSubscriptions.length > 0 ? (
                  <div className="space-y-3">
                    {sortedSubscriptions.slice(0, 6).map((subscription) => (
                      <div key={subscription._id} className="flex justify-between items-center bg-gray-50 p-2 rounded">
                        <div>
                          <div className="font-medium">{subscription.name}</div>
                          <div className="text-xs text-muted-foreground">{subscription.billingDate} 請求</div>
                        </div>
                        <Badge variant={subscription.amount > 5000 ? "destructive" : "outline"}>
                          {subscription.amount.toLocaleString()}円
                        </Badge>
                      </div>
                    ))}
                    ...
                    {sortedSubscriptions.length > 6 && (
                      <Button variant="outline" className="w-full mt-2" onClick={() => setActiveTab("analysis")}>
                        すべて表示
                      </Button>
                    )}
                  </div>
                ) : (
                  <p className="text-sm text-muted-foreground">サブスクリプションはありません</p>
                )}
              </CardContent>
            </Card>
          </div>
      ...
      <TabsContent value="analysis" className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              毎月の固定費の合計: {monthlySummary.fixedCosts.toLocaleString()}円
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                {withdrawals.length > 0 ? (
                  <div className="grid grid-cols-1 gap-2">
                    {withdrawals
                      ...
                      .map((withdrawal) => (
                        <div key={withdrawal._id} className="flex justify-between items-center p-3 border rounded">
                          <div>
                            <div className="font-medium">{withdrawal.description}</div>
                            <div className="text-xs text-muted-foreground">
                              {withdrawal.bank} {withdrawal.branch} • {format(parseISO(withdrawal.date), "yyyy年MM月dd日", { locale: ja })}
                            </div>
                          </div>
                  ...
                  <p className="text-muted-foreground">引き落としが登録されていません</p>
                )}
              </div>
              
              <div>
                <h3 className="text-lg font-medium mb-2">サブスクリプション一覧</h3>
                {subscriptions.length > 0 ? (
                  <div className="grid grid-cols-1 gap-2">
                    {sortedSubscriptions.map((subscription) => (
                      <div key={subscription._id} className="flex justify-between items-center p-3 border rounded">
                        <div>
                          <div className="font-medium">{subscription.name}</div>
                          <div className="text-xs text-muted-foreground">
                            タイプ: {subscription.type} • 請求日: {subscription.billingDate}
                          </div>
                        </div>
                        <Badge variant={subscription.amount > 5000 ? "destructive" : "outline"}>
                          {subscription.amount.toLocaleString()}円
                  ...
                  <p className="text-muted-foreground">サブスクリプションが登録されていません</p>
                )}
              </div>
            </div>
          </CardContent>
