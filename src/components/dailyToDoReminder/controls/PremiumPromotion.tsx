...
import { BadgeType } from './PlanBadge';
...
export const PremiumPromotion: React.FC<PremiumPromotionProps> = ({
  ...
  const keyFeatures = [
    ...
    },
    {
      title: '詳細な分析ダッシュボード',
      description:
        '生産性トレンド、目標達成率、時間効率などの詳細な分析により、仕事の進捗を可視化します。',
  ...
  const planDetails = {
    free: {
      ...
      features: ['最大100タスク', '基本的なタスク管理', '基本的なカレンダー表示', '簡易なレポート'],
      buttonText: '現在のプラン',
    ...
    basic: {
      ...
      features: ['最大1,000タスク', 'データエクスポート', '自動優先度調整', '基本的な統計機能'],
      buttonText: 'アップグレード',
      buttonVariant: 'default' as const,
      badgeType: (shouldShowPromo() ? 'discount' : undefined) as BadgeType | undefined,
    ...
    professional: {
      ...
      badgeType: (shouldShowPromo() ? 'best-value' : 'popular') as BadgeType,
    },
    enterprise: {
      title: 'エンタープライズ',
  ...
  return (
    <div className="space-y-8">
          ...
          {(['free', 'basic', 'professional', 'enterprise'] as PlanType[]).map((plan) => {
            ...
            return (
              <PremiumPlanCard
                ...
                showCountdown={
                  showCountdown &&
                  (('isRecommended' in details && details.isRecommended) ||
                    ('badgeType' in details && details.badgeType === 'limited'))
                }
                badgeType={'badgeType' in details ? (details.badgeType as BadgeType) : undefined}
      ...
      {/* 機能比較表 */}
      <PlanFeaturesComparison features={planFeatures} initialShowCount={compareFeatureCount} />
