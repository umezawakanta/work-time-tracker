...
import { ApiResponse, RequestConfig } from './ApiClient';
...
export interface ApiPlugin {
   ...
   */
  postRequest?: <T>(responseInfo: ApiResponseInfo<T>) => Promise<ApiResponseInfo<T>> | ApiResponseInfo<T>;
   ...
   * @returns 変更されたリクエスト情報（または元のリクエスト情報）
   */
  requestRetry?: (requestInfo: ApiRequestInfo, retryCount: number) => Promise<ApiRequestInfo> | ApiRequestInfo;
}
...
export class CachePlugin extends BaseApiPlugin {
  ...
  postRequest<T>(responseInfo: ApiResponseInfo<T>): ApiResponseInfo<T> {
    ...
    this.saveToCache(
      cacheKey,
      response,
      requestInfo.config.cacheTTL || this.defaultTTL
    );
