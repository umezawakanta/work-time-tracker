...
export class RateLimitManager {
  ...
  constructor() {
    ...
    this.userPlan = 'free' as SubscriptionPlan;
  ...
  public setResetInterval(intervalMs: number): void {
    ...
    this.logger.info(
      `繝ｬ繝ｼ繝亥宛髯舌・繝ｪ繧ｻ繝・ヨ髢馴囈繧・{intervalMs}繝溘Μ遘偵↓險ｭ螳壹＠縺ｾ縺励◆`
    );
  }
  ...
  public setCustomLimit(serviceKey: string, limit: number): void {
    ...
    this.logger.debug(
      `繧ｵ繝ｼ繝薙せ縲・{serviceKey}縲阪・繧ｫ繧ｹ繧ｿ繝繝ｬ繝ｼ繝亥宛髯舌ｒ${limit}縺ｫ險ｭ螳壹＠縺ｾ縺励◆`
    );
  }
  ...
  public removeCustomLimit(serviceKey: string): void {
    ...
    this.logger.debug(
      `繧ｵ繝ｼ繝薙せ縲・{serviceKey}縲阪・繧ｫ繧ｹ繧ｿ繝繝ｬ繝ｼ繝亥宛髯舌ｒ蜑企勁縺励∪縺励◆`
    );
  }
  ...
  public checkRateLimit(serviceName: string): RateLimitResult {
    ...
    const _currentCount = this.rateLimitCounter.get(_key) || 0;
    const _limit = this.getPlanLimit(serviceName);
    const _resetTime = this.lastRateLimitReset + this.resetInterval;

    // 邨先棡繧ｪ繝悶ず繧ｧ繧ｯ繝・
    const result: RateLimitResult = {
      allowed: _currentCount < _limit,
      limit: _limit,
      currentCount: _currentCount,
      resetTime: _resetTime,
      remaining: _limit - _currentCount,
    };

    if (_currentCount >= _limit) {
      this.logger.warn(
        `繝ｬ繝ｼ繝亥宛髯舌ｒ雜・℃縺励∪縺励◆: ${_currentCount}/${_limit} (${this.userPlan}繝励Λ繝ｳ)`,
        {
          service: serviceName,
          plan: this.userPlan,
        }
    ...
    // 繧ｫ繧ｦ繝ｳ繧ｿ繝ｼ繧貞｢怜刈
    this.rateLimitCounter.set(_key, _currentCount + 1);
    return result;
  }
  ...
  public getTimeUntilReset(): number {
    ...
    return Math.max(0, _resetTime - _now);
  }
