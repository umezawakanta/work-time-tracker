...
export class AIModelRegistry {
  ...
  private registerDefaultModels(): void {
    this.models = [
      ...
      },
      {
        ...
        maxTokens: 2048,
        provider: 'google',
        capabilities: ['content-generation', 'entity-extraction'],
        tokenCost: {
          input: 0.000006,
          output: 0.000018,
        },
  ...
  public getHighestPriorityModel(enhancementType?: string): AIModel | undefined {
    ...
    // 優先度順にソート（undefined の場合は 0 として扱う）
  ...
  public getModelSummaries(): AIModelSummary[] {
    return this.models.map((model) => ({
      ...
      description: `${model.provider} model with ${model.capabilities.length} capabilities`,
      id: model.id,
      name: model.name,
