...
export const StatisticsDetail: React.FC<StatisticsDetailProps> = ({ stats }) => {
  ...
  const inputPercentage = totalTypeCount > 0 
    ? Math.round((stats.inputTasks / totalTypeCount) * 100) 
    : 50;
  const outputPercentage = totalTypeCount > 0 
    ? Math.round((stats.outputTasks / totalTypeCount) * 100) 
    : 50;
  
  // 期限遵守率の計算
  const totalDeadlineTasks = stats.tasksCompletedBeforeDeadline + stats.tasksCompletedAfterDeadline;
  const beforeDeadlinePercentage = totalDeadlineTasks > 0
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="bg-gray-50 p-2 rounded">
            ...
            <div className="flex justify-between items-center">
              <p className={`text-sm font-medium ${getCompletionIndicatorClass(stats.completionRate)}`}>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-blue-50 p-2 rounded">
            ...
            <p className="text-sm font-medium text-blue-700">{stats.inputTasks} ({inputPercentage}%)</p>
          </div>
          
          <div className="bg-green-50 p-2 rounded">
            <p className="text-xs text-gray-500">アウトプットタスク</p>
            <p className="text-sm font-medium text-green-700">{stats.outputTasks} ({outputPercentage}%)</p>
          </div>
        </div>
