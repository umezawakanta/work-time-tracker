...
import { Plus, CheckCircle, Target, Sparkles, BookOpen, Lightbulb } from 'lucide-react';
 ...
 */
export const EmptyState: React.FC<EmptyStateProps> = ({ isPremium = false, onAnalyzeRequest }) => {
  const suggestions = [
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-br from-gray-50 to-blue-50 border-gray-200">
        <CardContent className="p-8 text-center">
          <div className="mb-6">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">今日のタスクがありません</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {suggestions.map((suggestion, index) => (
              ...
              >
                <suggestion.icon
                  className={`h-4 w-4 ${
                    suggestion.type === 'input' ? 'text-blue-500' : 'text-orange-500'
                  }`}
                />
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    >
                      {suggestion.type === 'input' ? 'インプット' : 'アウトプット'}
                    </span>
                  </div>
                </div>
              </button>
        ...
        <Card className="bg-gradient-to-r from-amber-50 to-yellow-50 border-amber-200">
          <CardContent className="p-6">
            <div className="text-center mb-4">
              ...
              <h4 className="font-medium text-amber-900">Premiumで更に便利に</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div className="text-center p-3 bg-white rounded-lg border border-amber-200">
                ...
                <p className="text-xs text-amber-700">タスクパターンを分析し最適化提案</p>
              </div>
              <div className="text-center p-3 bg-white rounded-lg border border-amber-200">
                <CheckCircle className="h-5 w-5 mx-auto text-amber-600 mb-2" />
                <h5 className="text-sm font-medium text-amber-900">無制限タスク</h5>
                <p className="text-xs text-amber-700">制限なしでタスクを管理</p>
              </div>
              <div className="text-center p-3 bg-white rounded-lg border border-amber-200">
                <Lightbulb className="h-5 w-5 mx-auto text-amber-600 mb-2" />
                <h5 className="text-sm font-medium text-amber-900">カスタム機能</h5>
                <p className="text-xs text-amber-700">カテゴリ、タグ、期限設定</p>
              </div>
            </div>
      ...
      <div className="text-center text-sm text-gray-500">
        <p>💡 生産性のコツ: インプット（学習）とアウトプット（実践）を7:3のバランスで</p>
      </div>
    </div>
  );
};
