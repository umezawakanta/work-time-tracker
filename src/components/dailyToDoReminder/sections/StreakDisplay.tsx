...
export default function StreakDisplay({ streakCount }: StreakDisplayProps) {
  ...
  const getStreakMessage = () => {
    if (streakCount === 0)
      return 'まだストリークがありません。タスクを完了して習慣を作りましょう！';
    if (streakCount === 1) return '初日達成！継続は力なり、明日も頑張りましょう！';
    if (streakCount < 3)
      return `${streakCount}日連続でタスクを完了しています！この調子で続けましょう！`;
    ...
    if (streakCount < 30)
      return `${streakCount}日連続達成！二週間を超えると習慣化の成功率が大幅に上がります！`;
    if (streakCount < 60) return `${streakCount}日連続達成！一ヶ月以上の継続は大変な実績です！`;
    if (streakCount < 100)
      return `${streakCount}日連続達成！二ヶ月以上継続できる人は全体の5%以下です！`;
