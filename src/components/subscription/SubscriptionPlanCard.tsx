    ...
    >
      <CardHeader>
        <div className="flex justify-between items-start">
          ...
          {isPopular && <Badge className="bg-primary hover:bg-primary/90">人気</Badge>}
          {isCurrent && <Badge className="bg-green-500 hover:bg-green-600">現在のプラン</Badge>}
        </div>
      </CardHeader>
          ...
          {features.map((feature, index) => (
            <li key={index} className="flex items-start">
              <div className="flex-shrink-0 mt-0.5">
                <Check
                  className={cn('h-5 w-5', feature.included ? 'text-green-500' : 'text-gray-300')}
                />
              </div>
              <div className="ml-2 flex items-center">
                <span className={cn('text-sm', !feature.included && 'text-gray-500 line-through')}>
                  {feature.name}
                </span>
                {feature.info && (
                  <TooltipProvider>
