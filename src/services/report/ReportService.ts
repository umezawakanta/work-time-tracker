...
class ReportService {
    ...
    private filterTodosByDateRange(
        todos: Todo[],
        startDate: string,
        endDate: string
    ): Todo[] {
    ...
    private calculateEfficiency(completedTodos: Todo[]) {
        ...
        const averageCompletionTime = tasksWithTime.length > 0
            ? tasksWithTime.reduce((sum, t) => sum + (t.efficiency?.completionTime || 0), 0) / tasksWithTime.length
            : 0;

        const fastestTask = tasksWithTime.length > 0
            ? tasksWithTime.reduce((fastest, current) =>
                (current.efficiency?.completionTime || 0) < (fastest.efficiency?.completionTime || Infinity)
                    ? current : fastest)
            : null;

        const slowestTask = tasksWithTime.length > 0
        ...
        tasksWithTime.forEach(task => {
            ...
            timePerCategory[category] = (timePerCategory[category] || 0) +
                (task.efficiency?.completionTime || 0);
        });
    ...
    private generateInsights(
        ...
        // 最も生産的な時間帯
        const mostProductiveHour = patterns.dailyPattern
    ...
    private generateRecommendations(
        ...
        // 曜日パターンに基づく推奨事項
        const mostProductiveDay = patterns.weeklyPattern
            .reduce((max, current) => current.count > max.count ? current : max);
