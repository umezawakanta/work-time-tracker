...
export class CacheAnalytics {
   ...
   * パフォーマンス記録を追加
   */
  public recordPerformance(type: AIEnhancementType, latency: number, hit: boolean): void {
   ...
   * キャッシュ分析を実行
   */
  public analyzeCache(status: CacheStatus, stats: CacheStats): CacheAnalyticsResult {
    // ヒット率の計算
    ...
    if ('cacheHits' in stats && stats.cacheHits && typeof stats.cacheHits === 'object') {
      // 型別のヒット数を集計
      const totalHits = Object.values(stats.cacheHits as Record<string, number>).reduce<number>(
        (sum, count) => sum + count,
        0
      );
    ...
    if ('cacheHits' in stats && stats.cacheHits && typeof stats.cacheHits === 'object') {
      Object.entries(stats.cacheHits as Record<string, number>).forEach(([type, count]) => {
        ...
        // 不明なタイプの場合はデフォルト値を使用
        const responseTime =
          this.estimatedResponseTimes[typeKey] || this.estimatedResponseTimes.custom;
    ...
    const p95Latency =
      latencies.length > 0 ? latencies[p95Index] || latencies[latencies.length - 1] : 0;
    const p99Latency =
      latencies.length > 0 ? latencies[p99Index] || latencies[latencies.length - 1] : 0;
    ...
    if (hitRate < 0.1) {
      recommendations.push(
        'キャッシュヒット率が非常に低いです。TTLを延長することを検討してください。'
      );
    }

    if (status.size < 10) {
      recommendations.push(
        'キャッシュサイズが小さいです。最大サイズを増やして効果を高めることを検討してください。'
      );
    }

    if (
      hitRate > 0.8 &&
      Object.keys(distribution).filter((key) => distribution[key as AIEnhancementType] > 0).length <
        3
    ) {
      recommendations.push(
        '特定のタイプに偏ったキャッシュ利用が見られます。他のタイプでもキャッシュの活用を検討してください。'
      );
    }
  ...
  public analyzeTimeRange(
    ...
    // 平均レイテンシーの計算
    const averageLatency =
      entriesInRange.reduce((sum, entry) => sum + entry.latency, 0) / entriesInRange.length;
