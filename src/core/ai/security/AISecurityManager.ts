...
export class AISecurityManager {
  ...
  private loadFilterConfig(): void {
    ...
    try {
      if (typeof localStorage !== 'undefined') {
        ...
        if (savedConfig) {
          const parsedConfig = JSON.parse(savedConfig) as Record<
            SecurityFilterType,
   ...
   * リクエストの検証
   */
  public async validateRequest(data: unknown, options: AIFeatureOptions): Promise<void> {
    if (!this.initialized) {
      this.initialize();
    }
   ...
   * コンテンツの検証
   */
  private async validateContent(data: unknown, _options: AIFeatureOptions): Promise<void> {
    // データが文字列の場合は直接チェック
  ...
  private async checkContentSafety(content: string): Promise<void> {
    ...
    if (
      ...
      this.filters['content-moderation'].enabled
    ) {
