...
export function applyExtensionMethods(apiClass: any): void {
            ...
            try {
                ...
                switch (request.method.toUpperCase()) {
                    case 'GET':
                        ...
                        break;
                    case 'POST':
                        response = await apiClass.post<T>(
                            request.endpoint,
                            request.data,
                            { ...config, ...request.config }
                        );
                    ...
                    case 'PUT':
                        ...
                        break;
                    case 'DELETE':
                        response = await apiClass.delete<T>(
                            request.endpoint,
                            { ...config, ...request.config }
                        );
                        break;
                    case 'PATCH':
                        response = await apiClass.patch<T>(
                            request.endpoint,
                            request.data,
                            { ...config, ...request.config }
                        );
    ...
    apiClass.getCached = async function <T>(
        ...
        const response = await apiClass.get<T>(endpoint, params, options?.config);
        ...
        const pollPromise = new Promise<ApiResponse<T>>((resolve, reject) => {
            const executePoll = async () => {
                ...
                try {
                    ...
                    // リクエストを実行
                    const response = await apiClass.get<T>(endpoint, options.params, options.config);
    ...
    apiClass.healthCheck = async function (
        ...
        try {
            ...
            if (response.success) {
                // サービス状態をレスポンスから取得
                const services = (response.data as Record<string, unknown>)?.services as Record<string, boolean> || {};
        ...
        if (options?.reconnect) {
            ...
            const handleClose = (event: CloseEvent) => {
                ...
                if (reconnectAttempts < maxReconnectAttempts) {
                    ...
                    apiClass.logger.info(`WebSocketが切断されました。${reconnectInterval}ms後に再接続を試みます (${reconnectAttempts}/${maxReconnectAttempts})`);
                    ...
                    apiClass.logger.warn(`WebSocketの再接続に${maxReconnectAttempts}回失敗しました。再接続を中止します。`);
                }
            };
    ...
    // 認証情報の更新
    apiClass.setCredentials = function (
        username: string,
        password: string
    ): void {
        ...
        while (attempt <= maxRetries) {
            try {
                ...
                if (options?.onRetry) {
                    ...
                    } as ApiResponse<T>, attempt + 1);
                }

                // ログにエラーとリトライを記録
                apiClass.logger.warn(`リクエストエラー: ${error instanceof Error ? error.message : String(error)}。リトライします (${attempt + 1}/${maxRetries})`);
