...
router.get('/', async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const supportRates = await SupportRate.find().populate('partyId').sort({ surveyId: -1 });
...
router.get('/:id', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    console.log(
      'All survey IDs in database:',
      allSurveyIds.map((s) => s._id.toString())
    );
