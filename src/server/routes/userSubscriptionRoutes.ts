...
const validateUserSubscription = [
  ...
  body('status')
    ...
    .withMessage("ステータスは 'active', 'canceled', 'expired' のいずれかである必要があります"),
  body('currentPeriodEnd')
    .isISO8601()
    .withMessage('currentPeriodEndは有効な日付である必要があります'),
];
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (!userSubscription) {
        res
          .status(404)
          .json({ message: '指定されたユーザーのサブスクリプション情報が見つかりません' });
        return;
      }
...
router.patch(
  ...
  body('cancelAtPeriodEnd')
    .isBoolean()
    .withMessage('cancelAtPeriodEndはブール値である必要があります'),
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      res.status(400).json({ errors: errors.array() });
...
router.delete('/:id', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const deletedUserSubscription = await UserSubscription.findByIdAndDelete(req.params.id);
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      const sanitizedPaymentMethod = {
        ...
        lastFour:
          typeof paymentMethod.cardNumber === 'string' ? paymentMethod.cardNumber.slice(-4) : null,
        expiryDate: typeof paymentMethod.expiryDate === 'string' ? paymentMethod.expiryDate : null,
        cardholderName:
          typeof paymentMethod.cardholderName === 'string' ? paymentMethod.cardholderName : null,
        isDefault: true,
      };
