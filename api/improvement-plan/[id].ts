...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const itemIndex = mockImprovementItems.findIndex((item) => item.id === id);

    if (itemIndex === -1) {
      res.status(404).json({ error: 'Improvement item not found' });
      return;
    }
    ...
    switch (req.method) {
      case 'GET':
        ...
        res.status(200).json({
          success: true,
          data: mockImprovementItems[itemIndex],
        });
        break;
      ...
      case 'PUT':
        ...
        const updatedItem: ImprovementItem = {
          ...mockImprovementItems[itemIndex],
          ...req.body,
          id, // IDは変更不可
          updatedAt: new Date().toISOString(),
        };

        mockImprovementItems[itemIndex] = updatedItem;
      ...
      case 'DELETE':
        // 改善項目を削除
        mockImprovementItems.splice(itemIndex, 1);

        res.status(200).json({
          success: true,
          message: 'Improvement item deleted successfully',
        });
        break;
    ...
    console.error('Improvement plan item API error:', error);
    res.status(500).json({
      success: false,
